...
module.exports = createCoreController('api::order.order', ({ strapi }) => ({
    ...
    createOrderWithCartItem: async (ctx) => {
        ...
        if (
            !Array.isArray(ctx.request.body.item) ||
        ...
        const shop = await strapi.entityService.findMany(
            'api::store-address.store-address'
        );
    ...
    cancelOrder: async (ctx) => {
        ...
        return await strapi.db
            ...
            .update({ where: { id: order.id }, data: { status: 'đã hủy' } });
    },
}));
