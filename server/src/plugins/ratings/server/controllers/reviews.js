...
module.exports = {
  ...
  async getUserReview(ctx) {
    ...
    const review = await strapi.db.query("plugin::ratings.review").findMany({
      select: ["id", "createdAt", "comment", "score"],
      where: {
        related_to: {slug},
        author: user.id
  ...
  async update(ctx) {
    ...
    const review = await strapi.entityService.findOne("plugin::ratings.review", id,
      {
        populate: {
          author: true,
          related_to: true
    ...
    const {comment, score} = ctx.request.body
    if (!score) {
      return ctx.badRequest("Score should be between 1-5", {comment, score})
    }
    // Update review
    await strapi.entityService.update("plugin::ratings.review", ctx.params.id, {
      data: {
        comment,
        score
      }
